...
class UNet3D(nn.Module):
    ...
    def __init__(self, n_channels, n_out):
        ...
        self.skip_conv1 = SkipConv(1024, 1024)
        self.up1 = Up(1024, 256)
        self.skip_conv2 = SkipConv(512, 512)
        self.up2 = Up(512, 128)
        self.skip_conv3 = SkipConv(256, 256)
        self.up3 = Up(256, 64)
        self.skip_conv1 = SkipConv(128, 128)
        self.up4 = Up(128, 64)
    ...
    def forward(self, x):
        ...
        x = self.up1(x, skip_conv1(x4))
        #print('Up 1')
        #print(x.shape)
        x = self.up2(x, skip_conv2(x3))
        #print('Up 2')
        #print(x.shape)
        x = self.up3(x, skip_conv3(x2))
        #print('Up 3')
        #print(x.shape)
        x = self.up4(x, skip_conv4(x1))
        #print('Up 4')
        #print(x.shape)
        x = self.out_conv(x)
        #print('Out')
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, bilinear=True, kernel=3):
        ...
        self.conv_skip = DoubleConv(6, out_channels, kernel) #ajustar
